-- ════════════════════════════════════════════════════════════════════════
-- "Any time" becomes a third way to schedule, not a switch hanging off one.
--
-- A grid plus a separate "but also accept other times" toggle described the
-- implementation, not the decision. The decision a business actually makes
-- is one of three:
--
--   grid   — only every N minutes, and nothing in between
--   any    — N-minute times are what we suggest, but name any minute
--   fixed  — it starts when it starts
--
-- So slot_mode carries all three and allow_any_time goes away.
-- ════════════════════════════════════════════════════════════════════════

alter table public.services drop constraint if exists services_slot_mode_check;
alter table public.services
  add constraint services_slot_mode_check
  check (slot_mode in ('grid', 'any', 'fixed'));

-- Everything that accepted off-grid times is exactly the new 'any'.
update public.services
   set slot_mode = 'any'
 where slot_mode = 'grid' and allow_any_time;

alter table public.services drop column if exists allow_any_time;

comment on column public.services.slot_mode is
  'grid: only the N-minute times. any: those times are suggestions, any minute '
  'is bookable. fixed: only the listed start times.';
...
begin
  ...
  -- Only 'grid' treats the grid as a rule; 'any' treats it as a suggestion.
  if v_service.slot_mode = 'grid' then
